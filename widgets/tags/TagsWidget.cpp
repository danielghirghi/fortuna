...
#include "../../dialogs/tags/TagWidget.h"
    ...
    , ui(new Ui::TagsWidget)
{
    ...
    connect(ui->edtPesquisar, &QLineEdit::returnPressed, this, &TagsWidget::on_btnPesquisar_clicked);
}
...
void TagsWidget::carregarTags()
{
    ...
    ui->lblQuantidade->setText(QString::number(tags.count()) + " tags");
...
void TagsWidget::on_btnLimpar_clicked()
{
    ui->edtPesquisar->setText("");
    carregarTags();
}

void TagsWidget::on_btnPesquisar_clicked()
{
    ui->tblTags->setRowCount(0);
    ...
    TagsRepository repository;
    QList<Tag> tags = repository.pesquisar(busca);
